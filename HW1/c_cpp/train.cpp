...
int main(int argc, char** argv)
{
    ...
    double** gamma = alloc_2D_matrix(length, hmm->state_num);
    double*** epsilon = new double** [length];
    for (int i=0; i<length; ++i)
        epsilon[i] = alloc_2D_matrix(hmm->state_num, hmm->state_num);
    double* gamma_init = new double [hmm->state_num];
    double* gamma_sum = new double [hmm->state_num];
    double** gamma_observe = alloc_2D_matrix(hmm->observ_num, hmm->state_num);
    double** epsilon_sum = alloc_2D_matrix(hmm->state_num, hmm->state_num);
    ...
    for (int it=0; it<iteration; ++it) {
        for (int s=0; s<v_seq.size(); ++s) {
            ...
            calculate_gamma(alpha, beta, hmm->state_num, length, gamma);
